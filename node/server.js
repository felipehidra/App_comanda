...
function inicia_server(){
	...
	db.connect(function(err){
    	if (err){
			...
			log(1,"Erro ao Conectar no Banco de Dados");
		}else{
			console.log("Nucleo conectado ao banco de dados!");
			log(1,"Nucleo Conectado ao Banco de Dados");
		}
	})
	...
	app.listen(4000,function(){
		log(1,"Aplicativos iniciados na porta 4000");  
	...
	io.sockets.on('connection', function(socket){
		...
		let resultado = '';
		let dados = [];
        db.query("SELECT * FROM cadastro WHERE nome LIKE '%"+data.nome+"%';").on('result', function(data){
        ...
        db.query("SELECT nome FROM cadastro inner join comanda on cadastro.id = comanda.cadastro where comanda.comanda = '"+data.comanda+"';").on('result', function(data){
				resultado = data.length;
				nome = data.nome;
			}).on('end', function(){
	...
	function log(flag,msg){
		...
		fs.writeFile(__dirname+'/Log_Nucleo.txt', texto,{enconding:'utf-8',flag: 'a+'}, function (err) {
			if (err) throw err;
		});
	}
	console.log('###############################################################\n Nucleo Online!\n###############################################################');
